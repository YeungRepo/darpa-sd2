...
def reg_term(Wlist):
  output = tf.reduce_sum(input_tensor=tf.abs(Wlist[0]));
  for k in range(1,len(Wlist)):
    output += tf.reduce_sum(input_tensor=tf.abs(Wlist[k]));
  return output;


def quick_nstep_predict(Y_p_old,u_control_all_training,with_control,num_bas_obs,iter,output_file_dir):
  ...
  plt.xlabel('t');
  fig = plt.gcf();
  if not (os.path.exists(output_file_dir+'/PredictionsDuringTraining/')):
    os.makedirs(output_file_dir+'/PredictionsDuringTraining/');
  target_file = output_file_dir+'/PredictionsDuringTraining/'+data_suffix.split('/')[-2].replace('.pickle','')+'nstep_prediction' + repr(iter) + '.pdf';
...
def expose_deep_basis(z_list,num_bas_obs,deep_dict_size,iter_num,u,output_file_dir):
  ...
  for i in range(0,num_bas_obs):
    ...
    #plt.ylim([-2.0,2.0]);
    fig = plt.gcf();
    if not (os.path.exists(output_file_dir+'/BasisPlots/')):
      os.makedirs(output_file_dir+'/BasisPlots/');
    plt.savefig(output_file_dir+'/BasisPlots/'+'/phi_with_u' + repr(i) + '_iternum_' + repr(iter_num) + '.jpg');
...
def xavier_init(n_inputs, n_outputs, uniform=True):
  ...
  if uniform:
    ...
    return tf.compat.v1.random_uniform_initializer(-init_range, init_range)
  else:
...
def weight_variable(shape):
  ...
  return tf.Variable(tf.compat.v1.random.truncated_normal(shape, mean=0.0,stddev=std_dev,dtype=tf.float32));
    ...
    for k in range(0,n_depth):
        ...
        if (k==0):
            ...
            if activation_flag==1:# RELU
                z1 = tf.nn.dropout(tf.nn.relu(tf.matmul(input_var,W1)+b1),1 - (keep_prob));
            if activation_flag==2: #ELU 
                z1 = tf.nn.dropout(tf.nn.elu(tf.matmul(input_var,W1)+b1),1 - (keep_prob));
            if activation_flag==3: # tanh
                z1 = tf.nn.dropout(tf.nn.tanh(tf.matmul(input_var,W1)+b1),1 - (keep_prob));
        ...
        if not (k==0):
            ...
            if activation_flag==1:
                z_temp_list.append(tf.nn.dropout(tf.nn.relu(prev_layer_output),1 - (keep_prob)));
            if activation_flag==2:
                z_temp_list.append(tf.nn.dropout(tf.nn.elu(prev_layer_output),1 - (keep_prob)));
            if activation_flag==3:
                z_temp_list.append(tf.nn.dropout(tf.nn.tanh(prev_layer_output),1 - (keep_prob)));
...
def initialize_stateinclusive_tensorflow_graph(n_u,deep_dict_size,hv_list,W_list,b_list,keep_prob=1.0,activation_flag=1,res_net=0):
  ...
  for k in range(0,n_depth):
      if (k==0):
        ...
        b1 = b_list[k];
        if activation_flag==1:# RELU
          z1 = tf.nn.dropout(tf.nn.relu(tf.matmul(u,W1)+b1),1 - (keep_prob));
        if activation_flag==2: #ELU 
          z1 = tf.nn.dropout(tf.nn.elu(tf.matmul(u,W1)+b1),1 - (keep_prob));
        if activation_flag==3: # tanh
          z1 = tf.nn.dropout(tf.nn.tanh(tf.matmul(u,W1)+b1),1 - (keep_prob));
        ...
        z_list.append(z1);
          ...
          if res_net and k==(n_depth-2):
              prev_layer_output += tf.matmul(u,W1)+b1 #  this expression is not compatible for variable width nets (where each layer has a different width at inialization - okay with regularization and dropout afterwards though)              
          if activation_flag==1:
              z_list.append(tf.nn.dropout(tf.nn.relu(prev_layer_output),1 - (keep_prob)));

          if activation_flag==2:
              z_list.append(tf.nn.dropout(tf.nn.elu(prev_layer_output),1 - (keep_prob)));

          if activation_flag==3:
              z_list.append(tf.nn.dropout(tf.nn.tanh(prev_layer_output),1 - (keep_prob)));
...
def Deep_Output_KIC_Objective(all_psiXp,all_psiXf,Kx,all_psiUp,Ku,step_size,all_Yf,all_Yp,Wh,learn_controllable_Koopman=0):
   ...
   if learn_controllable_Koopman:
     ...
     Kut = tf.transpose(a=Ku);
     Kxt = tf.transpose(a=Kx);
     ctrb_matrix = Kut;
     for ind in range(1,n):
        ctrb_matrix = tf.concat([ctrb_matrix,tf.matmul(tf.pow(Kxt,ind),Kut)],axis=1);
     ctrbTctrb = tf.matmul(ctrb_matrix,tf.transpose(a=ctrb_matrix) );
     print(ctrbTctrb.get_shape())
     ctrb_s,ctrb_v = tf.linalg.eigh(ctrbTctrb);
     print(tf.norm(tensor=ctrb_s,ord=1))
   ...
   # SSE
   output_prediction_error = tf.norm(tensor=all_Yf - all_Yf_predicted,axis=[0,1],ord='fro')
   state_prediction_error = tf.norm(tensor=all_psiXf - all_psiXf_predicted,axis=[0,1],ord='fro')
   output_frobenius_norm = tf.norm(tensor=all_Yf,axis=[0,1],ord='fro')
   state_frobenius_norm = tf.norm(tensor=all_psiXf,axis=[0,1],ord='fro')

   tf_koopman_loss =  tf.reduce_mean(input_tensor=[output_prediction_error/output_frobenius_norm ,state_prediction_error/state_frobenius_norm]) #+ tf.norm(all_Yp - all_Yp_predicted,axis=[0,1],ord='fro')/tf.norm(all_Yp,axis=[0,1],ord='fro');
...
def Deep_Control_Koopman_Objective(psiyp,psiyf,Kx,psiu,Ku,step_size,learn_controllable_Koopman=0):
   ...
   tf_koopman_loss =  tf.reduce_mean(input_tensor=tf.norm(tensor=psiyf - forward_prediction_control,axis=[0,1],ord='fro'))#/tf.reduce_mean(tf.norm(psiyp,axis=[0,1],ord='fro'));   
   optimizer = tf.compat.v1.train.AdagradOptimizer(step_size).minimize(tf_koopman_loss);
   result = sess.run(tf.compat.v1.global_variables_initializer());
...
def Deep_Direct_Koopman_Objective(psiyp,psiyf,Kx,step_size,convex_basis=0,u=None):
   ...
   tf_koopman_loss = tf.reduce_mean(input_tensor=tf.norm(tensor=forward_prediction-psiyf,axis=[0,1],ord='fro') )#/tf.reduce_mean(tf.norm(psiyf,axis=[0,1],ord='fro'));
   if convex_basis == 1:
...
def instantiate_output_variables(Output_Dim,Koopman_Dim):
  ...
  y_output_p_var = tf.compat.v1.placeholder(tf.float32,shape=[None,Output_Dim])
  y_output_f_var = tf.compat.v1.placeholder(tf.float32,shape=[None,Output_Dim])
...
def dynamic_train_net(dict_train, dict_test, deep_koopman_loss, optimizer, train_error_threshold=1e-2,
             ...
             if train_user_choice not in NO_CHOICE_VALUES:
                 ...
                 if param_user_choice not in NO_CHOICE_VALUES:
                     ...
                     while(NOT_SATISFIED):
                         ...
                         if parameter_satisfaction not in NO_CHOICE_VALUES:
                             print(NO_CHOICE_VALUES)
                             NOT_SATISFIED = False
                         else:
                             print('*** Enter the parameters again ***')
                 #print(max_epochs)
                 all_histories, good_start,n_epochs_run = train_net(up_all_training, uf_all_training, deep_koopman_loss, optimizer, U_train,
                 ...
                 #all_histories, good_start,n_epochs_run = train_net_v2(dict_train['Xp'], dict_train['Xf'], deep_koopman_loss, optimizer,dict_train['Up'],
                                                       dict_train['Yp'],dict_train['Yf'], train_error_threshold,
                                                                    valid_error_threshold, valid_error_threshold,max_epochs, step_size_val,batch_size)
    ...
    if len(sys.argv)>2:
      dict_run_info = {}
      run_info_index = 0
      all_histories, good_start,n_epochs_run = train_net(up_all_training, uf_all_training, deep_koopman_loss, optimizer, U_train,dict_train['Yp'],dict_train['Yf'], train_error_threshold,valid_error_threshold, valid_error_threshold,max_epochs, step_size_val,batch_size)
      dict_run_info[run_info_index] = generate_hyperparam_entry(feed_dict_train,feed_dict_test,deep_koopman_loss,n_epochs_run,step_size_val,batch_size)
...
def train_net(u_all_training,y_all_training,mean_diff_nocovar,optimizer,u_control_all_training=None,Output_p_batch=None,Output_f_batch=None,train_error_thres=1e-2,valid_error_thres=1e-2,test_error_thres=1e-2,max_iters=100000,step_size_val=0.01,batch_size=30):
...
#def train_net( Xp, Xf, covariance structure matrix of Xp to Xf, 
  ...
  covar_model_history = [];
  covar_diff_history = [];
  print('Training Starts now' + repr(iter)+ ","+ repr(max_iters))
  while (((test_error>test_error_thres) or (valid_error > valid_error_thres)) and iter < max_iters):
    ...
    if iter%samplerate==0:
      ...
      if (iter%250==0) or (iter==1):
        plt.close();
        if plot_deep_basis:
          fig_hand = expose_deep_basis(psiypz_list,num_bas_obs,deep_dict_size,iter,yp_feed,output_file_directory);
          fig_hand = quick_nstep_predict(Y_p_old,u_control_all_training,with_control,num_bas_obs,iter,output_file_directory);
  ...
  plt.close();
  return all_histories,good_start,iter;
...
while good_start==0 and try_num < max_tries:
  ...
  with tf.device('/cpu:0'):
    ...
    all_histories, good_start,dict_run_info = dynamic_train_net(dict_train, dict_test, deep_koopman_loss, optimizer, train_error_threshold,valid_error_threshold, max_epochs, step_size_val,batch_size)

    #all_histories,good_start  = train_net(up_all_training,uf_all_training,deep_koopman_loss,optimizer,U_train,Out_p_train,Out_f_train,valid_error_threshold,test_error_threshold,max_epochs,step_size_val);
    # all_histories,good_start  = train_net(up_all_training,uf_all_training,deep_koopman_loss,optimizer,U_train,Out_p_train,Out_f_train,valid_error_threshold*.1,test_error_threshold*.1,max_epochs,step_size_val/10);
    # all_histories,good_start  = train_net(up_all_training,uf_all_training,deep_koopman_loss,optimizer,U_train,Out_p_train,Out_f_train,valid_error_threshold*.025,test_error_threshold*.025,max_epochs,step_size_val/1000);
    # all_histories,good_start  = train_net(up_all_training,uf_all_training,deep_koopman_loss,optimizer,U_train,Out_p_train,Out_f_train,valid_error_threshold*.025,test_error_threshold*.025,max_epochs,step_size_val/10000);
