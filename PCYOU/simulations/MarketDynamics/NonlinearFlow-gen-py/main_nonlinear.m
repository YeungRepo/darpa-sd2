...
%         pio = zeros(narea,1);
        rhopo=zeros(m,1);
        rhomo=zeros(m,1);
        xo = [omegao;thetao;valveo;pmecho;lambdao;rhopo;rhomo];
%       xo = [omegao;thetao;valveo;pmecho;lambdao;phio;pio;rhopo;rhomo];


        txf = @(t, x) md(t,x,n,m,deltaPm,barDeltaP,underDeltaP, M, D, C, Bij, Bij_nonlinear,Plink_orig, R, Tg, Tb, G, G_controlled,  L,PF, H, Hg, Hl, rm); 
