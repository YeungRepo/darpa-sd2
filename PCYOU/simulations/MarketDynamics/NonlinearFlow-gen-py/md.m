...
function y = md(~,x,n,m,deltaPm,barDeltaP,underDeltaP, M, D, C,Bij,  Bij_nonlinear,Plink_orig, R, Tg, Tb, G, G_controlled,  L, PF, H, Hg, Hl, rm)
...
lambda = x(3*g+n+1:3*g+n+1);
% phi = x(3*g+2*n+1:3*g+3*n);
% pi_area = x(3*g+3*n+1:3*g+3*n+narea);
rhop = x(3*g+n+2:3*g+n+1+m);
rhom = x(3*g+n+2+m:3*g+n+1+2*m);
...
omega = zeros(n,1);
omega(G) = omega_g;
omega(L) = D(L,L)\( pgen(L) + deltaPm(L) - C(L,:)*(diag(Bij_nonlinear)*sin(transpose(C)*theta) -Plink_orig) );
...
domega_g  = M(G,G)\(deltaPm(G) + pgen(G) - D(G,G)*omega_g - C(G,:)*(diag(Bij_nonlinear)*sin(transpose(C)*theta) -Plink_orig));
...
dlambda = k_lambda*( - sum(deltaPm) - sum(pgen) );
...
drhop   = k_rho*(transpose(H)*(pgen(rm)+deltaPm(rm)) - barDeltaP);
drhom   = k_rho*(underDeltaP - transpose(H)*(pgen(rm)+deltaPm(rm))); 
%projection
ix=find(rhop<=0);
if ~isempty(ix)
...
y=[domega_g;dtheta;dvalve;dpmech;dlambda;drhop;drhom];
