...
function y = md(t,x,n,m,deltaPm,barDeltaP,underDeltaP, M, D, C,Bij,  Bij_nonlinear,Plink_orig, R, Tg, Tb,Td, G, G_controlled,  L,L_controlled, PF,PFd, H, Hg, Hl, rm, Bjj, Tv, xd, xdt, Ef, Bij_vf,CQ)
%global thistory
%narea = size(hatC,1);
g = length(G);
%l = length(L);
...
omega = zeros(n,1);
omega(G) = omega_g;
omega(L) = D(L,L)\( pgen(L)  -load(L) - C(L,:)*(edgev.*(diag(Bij_vf)*sin(transpose(C)*theta)) -Plink_orig) );
...
domega_g  = M(G,G)\(  pgen(G) -load(G)- D(G,G)*omega_g - C(G,:)*(edgev.*(diag(Bij_vf)*sin(transpose(C)*theta)) -Plink_orig));
dtheta  = omega;                     % theta rad, omega: rad/(s)
%dvalve =  diag(Tg)\(-valve - (1./R) .* omega_g );
dvalve =  diag(Tg)\(-valve );
%dpmech  = zeros(g,1);
dpmech= diag(Tb)\(- pmech + valve);
dload= zeros(n,1);  %  diag(Td)\(-load + d_signal); 
...
y=[domega_g;dtheta;dvalve;dpmech;dload;dE];
