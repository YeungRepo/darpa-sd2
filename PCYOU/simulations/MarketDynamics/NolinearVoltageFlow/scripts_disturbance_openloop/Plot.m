...
Delta_omegafull(:,L)=(-(Delta_flow - repmat(transpose(Plink_orig),length(T),1))*transpose(C(L,:))   -Delta_load(:,L)  )/D(L,L);

Delta_psignal = - (1./repmat(transpose(R(G_controlled)),length(T),1)).*Delta_omega(:,G_controlled) ;   % cancel the oroginal droop control 
...
% valve positions
figure(3);hold on;
plot(T, Delta_valve(:,:),'lineWidth',1.5);
grid off;
%axis([0 150 59.5 60.1]);
xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
...
% mechanic power
figure(4);hold on;
plot(T, Delta_pmech(:,:),'lineWidth',1.5);
grid off;
%axis([0 150 59.5 60.1]);
xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
ylabel('Mechanical Power (pu)','fontname', 'Arial','fontsize',20);
set(gca, 'fontname','Arial','fontsize',20)
...
% plot([T(1) T(end)], [3 -3; 3 -3],'--k','lineWidth',1.0);
% grid off;
% %axis([0 150 59.5 60.1]);
% xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
% ylabel('Binding Line Flow (pu)','fontname', 'Arial','fontsize',20);
% set(gca, 'fontname','Arial','fontsize',20)
...
% load
figure(7);hold on;
plot(T, Delta_load(:,:),'lineWidth',1.5);
grid off;
%axis([0 150 59.5 60.1]);
xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
ylabel('Controllable Load (pu)','fontname', 'Arial','fontsize',20);
set(gca, 'fontname','Arial','fontsize',20)
% 
% % d_signal
% figure(8);hold on;
