...
Delta_lmp_full=repmat(transpose(pricefull),length(T),1);
Delta_lmp=repmat(transpose(price_signal),length(T),1);

Delta_psignal = Delta_lmp_full(:,bus_controlled) +  gain./(repmat(transpose(price_sup(bus_controlled)),length(T),1)-Delta_lmp_full(:,bus_controlled))  -  1./(k_control*repmat(transpose(PF),length(T),1)).* Delta_pmech(:,G_controlled)  ...
       +  Delta_pmech(:,G_controlled);   % cancel the oroginal droop control 

Delta_dsignal =1./(k_control*repmat(transpose(PFd),length(T),1)).* Delta_load(:,L_controlled)  - Delta_lmp_full(:,L_controlled) - gain./(repmat(transpose(price_sup(L_controlled)),length(T),1)-Delta_lmp_full(:,L_controlled))  ...
...
% frequencies
    ...
    set(gca, 'fontname','Arial','fontsize',20,'xlim',[1,50])
...
if rest_plots
    ...
    figure(10);hold on;
    plot(T, Delta_lmp(:,:),'lineWidth',1.5);
    grid off;
    %axis([0 150 59.5 60.1]);
    xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
