...
price_sup[np.concatenate((np.transpose(bus_controlled-1) , L_controlled-1),axis=0)]=50*np.ones((5,1))
gain=500.0
...
def md(t,x,n,m,deltaPm,barDeltaP,underDeltaP, M, D, C,Bij,  Bij_nonlinear,Plink_orig, R, Tg, Tb,Td, G, G_controlled,  L,L_controlled, PF,PFd, H, Hg, Hl, rm, Bjj, Tv, xd, xdt, Ef, Bij_vf,CQ,pricefull, bus_controlled,price_sup,gain):
	...
	p_signal[G_controlled-1] = pricefull[bus_controlled] +  np.divide(gain*np.ones((bus_controlled.shape[0],1)),(price_sup[bus_controlled-1]-pricefull[bus_controlled-1]) )  -  np.multiply(np.divide(1.0,(a1)), pmech[G_controlled-1])+ pmech[G_controlled]
...
this_ode.set_integrator("vode",nsteps=500,method='bdf')  # choice of method
