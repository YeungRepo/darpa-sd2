...
price_signal=np.random.rand(G_controlled.shape[0]+L_controlled.shape[0],1)*80-40
...
def md(t,x,n,m,deltaPm,barDeltaP,underDeltaP, M, D, C,Bij,  Bij_nonlinear,Plink_orig, R, Tg, Tb,Td, G, G_controlled,  L,L_controlled, PF,PFd, H, Hg, Hl, rm, Bjj, Tv, xd, xdt, Ef, Bij_vf,CQ,pricefull, bus_controlled,price_sup,gain):
	...
	k_lambda =1/np.mean(np.diag(M[G-1,G-1]))*0.4
	k_rho = 25*0.5*np.identity(m)
	omegarange = range(g)
	thetarange = range(g,g+n)
	valverange = range(n+g,n+g+g)
	pmechrange = range(n+2*g,n+2*g+g)
	loadrange = range(n+3*g,n+3*g+n)
	Erange = range(2*n+3*g,2*n+3*g+n)
	...
	p_signal[G_controlled-1] = pricefull[bus_controlled-1] +  np.divide(gain*np.ones((bus_controlled.shape[0],1)),(price_sup[bus_controlled-1]-pricefull[bus_controlled-1]) )  -  np.multiply(np.divide(1,(a1)), pmech[G_controlled-1])+ pmech[G_controlled-1]
	d_signal=np.zeros((n,1))
	d_signal[L_controlled-1]= np.multiply(np.divide(1,(a2)), loadLC) -  pricefull[L_controlled-1] -  np.divide(gain*np.ones((L_controlled.shape[0],1)),(price_sup[L_controlled-1]-pricefull[L_controlled-1])) +loadLC
	loadG = load[G-1]
...
x[0, :] = x0.ravel()
r = integrate.ode(md).set_integrator("vode")  # choice of method
