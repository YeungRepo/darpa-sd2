...
%----------------price signal
price_signal=rand([length(G_controlled)+length(L_controlled),1])*80-40;
pricefull=zeros(n,1);
%pricefull([transpose(bus_controlled),L_controlled])=price_signal;

price_sup=zeros(n,1);
price_sup([bus_controlled' , L_controlled])=[50; 50; 50; 50; 50];
gain=500;
...
%        global thistory 
%        thistory = [];
         txf = @(t, x) md(t,x,n,m,deltaPm,barDeltaP,underDeltaP, M, D, C, Bij, Bij_nonlinear,Plink_orig, R, Tg, Tb,Td, G, G_controlled,  L,L_controlled,PF,PFd, H, Hg, Hl, rm, Bjj, Tv, xd, xdt, Ef,Bij_vf,CQ, pricefull,bus_controlled,price_sup,gain); 
 %       txf = @(t, x) md(t,x,n,m,deltaPm,barDeltaP,underDeltaP, M, D, C, Bij, Bij_nonlinear,Plink_orig, R, Tg, Tb,Td, G, G_controlled,  L,L_controlled,PF,PFd, H, Hg, Hl, rm, Bjj, Tv, xd, xdt, Ef,Bij_vf,CQ,opts); 
        ...
        [T,X] = ode45(txf,[0:0.4:Tmax],xo,opts);    
...
figure(2)
plot(X(end-5:end));
outputfile=strcat(string(iter_test),'.csv');
