...
D=D*2;
D=D*3;
%D=D*2;   % for illustrative purposes
...
barP = 10*ones(m,1);                % Thermal limits
...
%         pio = zeros(narea,1);
        ...
        %psiuo = zeros(size(Bv,1),1);
%         xo = [omegao;thetao;valveo;pmecho;loado;lambdao;rhopo;rhomo;Eo];
        xo = [omegao;thetao;pmecho;loado;Eo];
        %xo = [omegao;thetao;pmecho;loado;Eo;psiuo];
%       xo = [omegao;thetao;valveo;pmecho;lambdao;phio;pio;rhopo;rhomo];
        
%----------------price signal
price_signal=rand([length(G_controlled)+length(L_controlled),1])*8-16;
pricefull=zeros(n,1);
pricefull([transpose(bus_controlled),L_controlled])=price_signal;

price_sup=zeros(n,1);
price_sup([bus_controlled' , L_controlled])=-ones(5,1)*8;
gain=500;
...
%         mass=diag([mass1;mass2]);
        ...
        opts.AbsTol=1e-3;
        opts.RelTol=1e-2;
        opts.MassSingular = true;
%        global thistory 
%        thistory = [];
         txf = @(t, x) md_gendata(t,x,n,m,deltaPm,barDeltaP,underDeltaP, M, D, C, Bij, Bij_nonlinear,Plink_orig, R, Tg, Tb,Td, G, G_controlled,  L,L_controlled,PF,PFd, H, Hg, Hl, rm, Bjj, Tv, xd, xdt, Ef,Bij_vf,CQ, pricefull,bus_controlled,price_sup,gain); 
...
Plot;
% 
output=[X, Delta_flowdeviation, repmat(transpose(price_signal),length(T),1)];
% hold on
% figure(1)
% plot(X(1:10))
% figure(2)
% plot(X(end-5:end));
outputfile=strcat('/Users/yeun026/Documents/darpa-sd2/deepDMD/dataset_NoImbalance_NoDroopControl/',string(iter_test),'.csv');
...
chdir('/Users/yeun026/Documents/darpa-sd2/deepDMD');
commandStr = 'python /Users/yeun026/Documents/darpa-sd2/deepDMD/gen_MD_data.py';
[status,commandOut] = system(commandStr);

chdir('/Users/yeun026/Documents/darpa-sd2/deepDMD');
commandStr = 'python /Users/yeun026/Documents/darpa-sd2/deepDMD/gen_control_Koopman.py';
[status,commandOut] = system(commandStr);

chdir('/Users/yeun026/Documents/darpa-sd2/deepDMD');
commandStr = 'python /Users/yeun026/Documents/darpa-sd2/deepDMD/H2synth_LMI.py';
[status,commandOut] = system(commandStr);

chdir('/Users/yeun026/Documents/darpa-sd2/PCYOU/simulations/MarketDynamics/NolinearVoltageFlow/scripts_openloop_control');
gen_continuous_Koopman_controller
gen_continuous_Koopman_model
