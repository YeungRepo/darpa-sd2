...
dtb=[30];
deltaPm=zeros(n,1);
% deltaPm(dtb)= trnd(1,length(dtb),1);  
deltaPm(dtb)= -1;
...
%     end;
    ...
    pmecho = zeros(g,1);   % mechanic power
    loado  = zeros(n,1);  %-----
...
%         
%         
%         [T,X] = ode23t(txf,[0:0.1:2*Tmax],xo,opts);    
%         
%         
% 
% 
%     traj_omega = X(:,1:g);
%     traj_theta = X(:,g+1:g+n);
%     traj_load  = X(:,3*g+n+1:3*g+2*n);
%     
%     traj_E    = X(:,3*g+2*n+2+2*m:3*g+3*n+1+2*m);
% 
%     traj_edgev=ones(length(T),m);
% for cnt1=1:m
%     for cnt2=1:n
%         if C(cnt2,cnt1) ~=0
%             traj_edgev(:,cnt1)=traj_edgev(:,cnt1).*traj_E(:,cnt2);
%         end
%     end
% end
% traj_flow=traj_edgev.*repmat(transpose(Bij_vf),length(T),1).*sin(traj_theta*C); 
% traj_omegafull=zeros(length(T),n);
% traj_omegafull(:,G)=traj_omega;
% traj_omegafull(:,L)=(-(traj_flow - repmat(transpose(Plink_orig),length(T),1))*transpose(C(L,:)) + repmat(transpose(deltaPm(L)),length(T),1)  - traj_load(:,L)  )/D(L,L);
% 
% 
% 
% 
%     traj_lambda = X(:,3*g+2*n+1:3*g+2*n+1);
%     traj_rhop = X(:,3*g + 2*n+2:3*g + 2*n+1+m);
%     traj_rhom = X(:,3*g + 2*n+2+m:3*g + 2*n+1+2*m); 
%     traj_lmp= repmat(traj_lambda,1,n-1) - traj_omegafull(:,rm) + traj_rhom * transpose(H) - traj_rhop * transpose(H) ;
% 
% 
% 
%     price0 =transpose( [0 traj_lmp(end,:)]);
%     
%     
%     
%      deltaPm(30)=-1;
...
%         
%         
%         [T,X] = ode23t(txf,[0:0.1:Tmax],xo,opts);   
%     
%     
%     
%     xo=X(end,:);
...
%     deltaPm(30)=-2;
% 
%     txf_spoofing = @(t, x) md_spoofing(t,x,n,m,deltaPm,barDeltaP,underDeltaP, M, D, C, Bij, Bij_nonlinear,Plink_orig, R, Tg, Tb,Td, G, G_controlled,  L,L_controlled,PF,PFd, H, Hg, Hl, rm, Bjj, Tv, xd, xdt, Ef,Bij_vf,CQ,price0,bus_controlled);     
%     [T,X] = ode23t(txf_spoofing,[0:0.1:Tmax],xo,opts);  
%     
% Plot;
%     
%     txf = @(t, x) md(t,x,n,m,deltaPm,barDeltaP,underDeltaP, M, D, C, Bij, Bij_nonlinear,Plink_orig, R, Tg, Tb,Td, G, G_controlled,  L,L_controlled,PF,PFd, H, Hg, Hl, rm, Bjj, Tv, xd, xdt, Ef,Bij_vf,CQ); 
%     [T,X1] = ode23t(txf,[0:0.1:Tmax],xo,opts);   
%     
% 
% Plot1;
...
deltaPm(30)=-1;
price_sup=zeros(n,1);
price_sup([bus_controlled' , L_controlled])=[50; 15; 15; 15;15];
gain=500;


    txf_spoofing = @(t, x) md_spoofing(t,x,n,m,deltaPm,barDeltaP,underDeltaP, M, D, C, Bij, Bij_nonlinear,Plink_orig, R, Tg, Tb,Td, G, G_controlled,  L,L_controlled,PF,PFd, H, Hg, Hl, rm, Bjj, Tv, xd, xdt, Ef,Bij_vf,CQ,price0,bus_controlled,price_sup,gain);     
    [T,X] = ode23t(txf_spoofing,[0:0.1:Tmax],xo,opts);  
