...
if h==3||h==4||h==6 ||h==7  % all cases of unified control
  % ------------     
    Delta_omega = X1(:,1:g);
    Delta_theta = X1(:,g+1:g+n);
    Delta_valve = X1(:,g+n+1:2*g+n);
    Delta_pmech = X1(:,2*g+n+1:3*g+n);
    Delta_load  = X1(:,3*g+n+1:3*g+2*n);
    
    Delta_lambda = X1(:,3*g+2*n+1:3*g+2*n+1);
    Delta_rhop = X1(:,3*g + 2*n+2:3*g + 2*n+1+m);
    Delta_rhom = X1(:,3*g + 2*n+2+m:3*g + 2*n+1+2*m);
    Delta_E    = X1(:,3*g+2*n+2+2*m:3*g+3*n+1+2*m);
 % ------------    
