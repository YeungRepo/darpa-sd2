...
% Delta_psignal = repmat( transpose(price0(bus_controlled)), length(T) ,1)  - 10./( repmat(Delta_lambda,1,length(G_controlled)) - Delta_omega(:,G_controlled) + Delta_rhom * transpose(Hg(G_controlled,:)) - Delta_rhop * transpose(Hg(G_controlled,:)) )  -  1./(k_control*repmat(transpose(PF),length(T),1)).* Delta_pmech(:,G_controlled)  ...
...
% 
% Delta_dsignal =1./(k_control*repmat(transpose(PFd),length(T),1)).* Delta_load(:,L_controlled) - repmat(transpose(price0(L_controlled)),length(T),1) + 10./( repmat(Delta_lambda,1,length(L_controlled)) - Delta_omegafull(:,L_controlled) + Delta_rhom * transpose(H(L_controlled-1,:)) - Delta_rhop * transpose(H(L_controlled-1,:)) )   ...
%      +  Delta_load(:,L_controlled);   
% 
% Delta_lmp= repmat(transpose(price0(rm)),length(T),1) -  10./ ( repmat(Delta_lambda,1,n-1) - Delta_omegafull(:,rm) + Delta_rhom * transpose(H) - Delta_rhop * transpose(H)   ) ;

Delta_psignal =( repmat(Delta_lambda,1,length(G_controlled)) - Delta_omega(:,G_controlled) + Delta_rhom * transpose(Hg(G_controlled,:)) - Delta_rhop * transpose(Hg(G_controlled,:)) )  + gain./( repmat(transpose(price_sup(bus_controlled)),length(T),1) -( repmat(Delta_lambda,1,length(G_controlled)) - Delta_omega(:,G_controlled) + Delta_rhom * transpose(Hg(G_controlled,:)) - Delta_rhop * transpose(Hg(G_controlled,:)) ))  -  1./(k_control*repmat(transpose(PF),length(T),1)).* Delta_pmech(:,G_controlled)  ...
    +1./repmat(transpose(R(G_controlled)),length(T),1).*Delta_omega(:,G_controlled)   +  Delta_pmech(:,G_controlled);   % cancel the oroginal droop control 

Delta_dsignal =1./(k_control*repmat(transpose(PFd),length(T),1)).* Delta_load(:,L_controlled) - ( repmat(Delta_lambda,1,length(L_controlled)) - Delta_omegafull(:,L_controlled) + Delta_rhom * transpose(H(L_controlled-1,:)) - Delta_rhop * transpose(H(L_controlled-1,:)) ) - gain./( repmat(transpose(price_sup(L_controlled)),length(T),1)   -  ( repmat(Delta_lambda,1,length(L_controlled)) - Delta_omegafull(:,L_controlled) + Delta_rhom * transpose(H(L_controlled-1,:)) - Delta_rhop * transpose(H(L_controlled-1,:)) ))   ...
     +  Delta_load(:,L_controlled); 

Delta_lmp= ( repmat(Delta_lambda,1,n-1) - Delta_omegafull(:,rm) + Delta_rhom * transpose(H) - Delta_rhop * transpose(H)   ) +  gain./ (repmat(transpose(price_sup(rm)),length(T),1)- ( repmat(Delta_lambda,1,n-1) - Delta_omegafull(:,rm) + Delta_rhom * transpose(H) - Delta_rhop * transpose(H)   ) );
Delta_lmp_true =  ( repmat(Delta_lambda,1,n-1) - Delta_omegafull(:,rm) + Delta_rhom * transpose(H) - Delta_rhop * transpose(H)   );
...
figure(11);hold on;
plot(T, Delta_lmp_true(:,[L_controlled,bus_controlled']-1),'lineWidth',1.5);
grid off;
%axis([0 150 59.5 60.1]);
xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
ylabel('Real DLMP ($/pu)','fontname', 'Arial','fontsize',20);
set(gca, 'fontname','Arial','fontsize',20)
