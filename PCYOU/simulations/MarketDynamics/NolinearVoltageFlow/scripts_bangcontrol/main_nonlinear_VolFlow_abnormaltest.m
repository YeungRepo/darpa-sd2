...
%         [T,X] = ode23t(txf,[0:0.3:Tmax],xo,opts);    
        
        
        
%% bang-control --- shed loads when frequency drop below a threshold
...
timestep=0.3;
counter=0;
while flag_bangtest_passed == 0
counter = counter +1;

txf = @(t, x) md(t,x,n,m,deltaPm,barDeltaP,underDeltaP, M, D, C, Bij, Bij_nonlinear,Plink_orig, R, Tg, Tb,Td, G, G_controlled,  L,L_controlled,PF,PFd, H, Hg, Hl, rm, Bjj, Tv, xd, xdt, Ef,Bij_vf,CQ); 

[T,X] = ode23t(txf,[0:timestep:(Tmax - time_passed) ],xo,opts); 
...
deltaPm_history=[deltaPm_history, deltaPm ];
freq_downthreshold = -0.1 * (2*pi);
freq_upthreshold = 0.1 * (2*pi);
shed_unit=1;

flag_shed=0;
for cntt=1:length(T)
    if min(traj_omegafull(cntt,:)) < freq_downthreshold
        [~,index] = min(traj_omegafull(cntt,:));
        deltaPm(index) = deltaPm(index) + shed_unit;
        flag_shed = 1;
    end
    
    if max(traj_omegafull(cntt,:)) > freq_upthreshold
        [~,index] = max(traj_omegafull(cntt,:));
        deltaPm(index) = deltaPm(index) - shed_unit;
        flag_shed = 1;
    end

%     for cntn=1:n
%         if traj_omegafull(cntt, cntn)  < freq_downthreshold
%             deltaPm(cntn) = deltaPm(cntn) + shed_unit;
%             flag_shed = 1;
%         end
%         if traj_omegafull(cntt, cntn) > freq_upthreshold
%             deltaPm(cntn) = deltaPm(cntn) - shed_unit;
%             flag_shed =1;
%         end
%     end 
...
cntt
complete_X = [ complete_X ; X(1:(cntt-1),:)];
    
xo = transpose(X(cntt,:));
time_passed = time_passed + (cntt-1) * timestep;
if Tmax - time_passed < timestep
    complete_X = [complete_X; X(cntt,:) ];
    break;
end

if flag_shed == 0
    flag_bangtest_passed =1;
end 
