...
if h==3||h==4||h==6 ||h==7  % all cases of unified control
  % ------------     
    Delta_omega = complete_X(:,1:g);
    Delta_theta = complete_X(:,g+1:g+n);
    Delta_valve = complete_X(:,g+n+1:2*g+n);
    Delta_pmech = complete_X(:,2*g+n+1:3*g+n);
    Delta_load  = complete_X(:,3*g+n+1:3*g+2*n);
    
    Delta_lambda = complete_X(:,3*g+2*n+1:3*g+2*n+1);
    Delta_rhop = complete_X(:,3*g + 2*n+2:3*g + 2*n+1+m);
    Delta_rhom = complete_X(:,3*g + 2*n+2+m:3*g + 2*n+1+2*m);
    Delta_E    = complete_X(:,3*g+2*n+2+2*m:3*g+3*n+1+2*m);
 % ------------    
...
[new_T,~] = size(complete_X);
newT_vec=0:timestep: (new_T-1) *timestep;
Delta_edgev=ones(new_T,m);
for cnt1=1:m
    for cnt2=1:n
        if C(cnt2,cnt1) ~=0
...
Delta_flow=Delta_edgev.*repmat(transpose(Bij_vf),new_T,1).*sin(Delta_theta*C); 
Delta_flowdeviation=Delta_flow - repmat(transpose(Plink_orig),new_T,1);

Delta_flownonlinear=repmat(transpose(Bij_nonlinear),new_T,1).*sin(Delta_theta*C);
Delta_flowlinear=repmat(transpose(Bij),new_T,1).*(Delta_theta*C);

      
Delta_omegafull=zeros(new_T,n);
Delta_omegafull(:,G)=Delta_omega;
Delta_omegafull(:,L)=(-(Delta_flow - repmat(transpose(Plink_orig),new_T,1))*transpose(C(L,:)) + repmat(transpose(deltaPm(L)),new_T,1)  - Delta_load(:,L)  )/D(L,L);

Delta_psignal = ( repmat(Delta_lambda,1,length(G_controlled)) - Delta_omega(:,G_controlled) + Delta_rhom * transpose(Hg(G_controlled,:)) - Delta_rhop * transpose(Hg(G_controlled,:)) )  -  1./(k_control*repmat(transpose(PF),new_T,1)).* Delta_pmech(:,G_controlled)  ...
    +1./repmat(transpose(R(G_controlled)),new_T,1).*Delta_omega(:,G_controlled)   +  Delta_pmech(:,G_controlled);   % cancel the oroginal droop control 

Delta_dsignal =1./(k_control*repmat(transpose(PFd),new_T,1)).* Delta_load(:,L_controlled)  - ( repmat(Delta_lambda,1,length(L_controlled)) - Delta_omegafull(:,L_controlled) + Delta_rhom * transpose(H(L_controlled-1,:)) - Delta_rhop * transpose(H(L_controlled-1,:)) )   ...
...
% frequencies
figure(1);hold on;
plot(newT_vec, Delta_omegafull(:,:)/(2*pi) + 60,'lineWidth',1.5);
plot([newT_vec(1) newT_vec(end)], [60.0 60.0],'--k','lineWidth',1.0);
grid off;
...
figure(2);hold on;
plot(newT_vec, Delta_theta(:,:),'lineWidth',1.5);
grid off;
%axis([0 150 59.5 60.1]);
xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
...
% valve positions
figure(3);hold on;
plot(newT_vec, Delta_valve(:,G_controlled),'lineWidth',1.5);
grid off;
%axis([0 150 59.5 60.1]);
xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
...
% mechanic power
figure(4);hold on;
plot(newT_vec, Delta_pmech(:,G_controlled),'lineWidth',1.5);
grid off;
%axis([0 150 59.5 60.1]);
xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
...
if h==2 ||h==3 || h==4 
    % input signal
    figure(5);hold on;
    plot(newT_vec, Delta_psignal,'lineWidth',1.5);
    grid off;
    %axis([0 150 59.5 60.1]);
    xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
...
% line flow
figure(6);hold on;
plot(newT_vec, Delta_flow(:,limitlink),'lineWidth',1.5);
grid off;
%axis([0 150 59.5 60.1]);
xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
...
% line flow
figure(16);hold on;
plot(newT_vec, Delta_flow(:,:),'lineWidth',1.5);
grid off;
%axis([0 150 59.5 60.1]);
xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
ylabel('Actual Line Flow (pu)','fontname', 'Arial','fontsize',20);
...
% load
figure(7);hold on;
plot(newT_vec, Delta_load(:,L_controlled),'lineWidth',1.5);
grid off;
%axis([0 150 59.5 60.1]);
xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
...
% d_signal
figure(8);hold on;
plot(newT_vec, Delta_dsignal,'lineWidth',1.5);
grid off;
%axis([0 150 59.5 60.1]);
xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
...
figure(9);hold on;
plot(newT_vec, Delta_E(:,G),'lineWidth',1.5);
grid off;
%axis([0 150 59.5 60.1]);
xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
...
figure(10);hold on;
plot(newT_vec, Delta_lmp(:,[L_controlled,bus_controlled']-1),'lineWidth',1.5);
grid off;
%axis([0 150 59.5 60.1]);
xlabel('Time (sec)','fontname', 'Arial','fontsize',20);
