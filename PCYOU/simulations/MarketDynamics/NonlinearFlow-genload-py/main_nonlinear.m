...
%         pio = zeros(narea,1);
        rhopo=zeros(m,1);
        rhomo=zeros(m,1);
        xo = [omegao;thetao;valveo;pmecho;loado;lambdao;rhopo;rhomo];
%       xo = [omegao;thetao;valveo;pmecho;lambdao;phio;pio;rhopo;rhomo];


        txf = @(t, x) md(t,x,n,m,deltaPm,barDeltaP,underDeltaP, M, D, C, Bij, Bij_nonlinear,Plink_orig, R, Tg, Tb,Td, G, G_controlled,  L,L_controlled,PF,PFd, H, Hg, Hl, rm); 
        [T,X] = ode23t(txf,[0 Tmax],xo);    
