...
def lsq_Koop_for_f(points, N_L, n, alpha, func, Mu, initer):
    """
    Calculates the centers for the koopman operator and weights for approximating f(x).
    :param points: 
    :param N_L: 
    ...
    L_T = np.array([[mv_log_prime(x, Mu, alpha, l, n, func) for l in range(N_L)] for x in points])
    ones = np.ndarray(shape=(len(points), 1), buffer=np.ones(len(points)))
    # print(ones.shape, np.array(points).shape, L_T.shape)
    # The Jacobian below is specific to the van der pol system
    jac_vdp = np.array()
    A = np.hstack([ones, np.array(points), L_T])
    W = np.zeros([n, N_L + n + 1])
    for row in range(n):
        b = np.array([func(x)[row] for x in points])
        W[row, :] = np.linalg.lstsq(A, b)[0]
    return W


def lsq_Mu_Koop(points, Mu, alpha, n, N_L):
    ...
    print("[INFO]: basis func_mat.shape: " + repr(basis_func_mat.shape));
    print("[INFO]: target_f.shape: " + repr(target_f.shape));
    ...
    try:
        ...
        inv_basis_func_mat = np.dot(np.transpose(basis_func_mat),
                                    np.linalg.inv(np.dot(basis_func_mat, np.transpose(basis_func_mat))));
    except:
    ...
    lsq_upper_err = np.linalg.norm(target_f - np.dot(K_upper_half, basis_func_mat), 'fro') / (
    target_f.shape[0] * target_f.shape[1]);

    print("Least squares error for K: " + repr(lsq_upper_err));

    print("[INFO] K_upper_half: " + repr(K_upper_half));



def lsq_Mu_Koop_for_f(points, N_L, n, alpha, func, Mu, initer):
    """
    Calculates the centers for the koopman operator and weights for approximating f(x).
    :param points: 
    :param N_L: 
    ...
    opt_results = optimize.minimize(lambda inpt: lsq_p_objective_func(points, Mu, inpt, alpha, func, N_L, n), x0=initer, method="COBYLA",
                                    options={"maxiter": 50, "disp": False})#, "ftol":1e-6}, callback=call)

    K = opt_results.x.reshape(n, N_L)
...
Mu = np.array([[all_x[i], all_y[i]] for i in range(0,len(all_x))])
...
for initer_iter in range(10):
    try:
        ...
        K_top = lsq_Koop_for_f(points, N_L, n, alpha, f, Mu,  initer[0: N_L * n])
...
atpt_num = str(12)  # Change this value to stop saving over yourself. (attempt number)
# Attempt 11 was real nice!
